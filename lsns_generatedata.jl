...
#Construct BNS instance for tstep - 6-12 hours
...
weekstart = DateTime(2019, 6, 30, 8) #weekstartlist[ex]
...
runid = string("initialsoln_ex", ex, "_tstep", tstep, "_rundate", today())
...
for subprob in subproblemlist
	...
	if sp_orders == []
		remove!(subproblemlist, subprob)
		global counter += 1
		println("------", counter, "------")
	else
		push!(timewindowlist, subprob["endtime"] - subprob["starttime"])
		push!(numlocslist, length(subprob["locgroup"]))
		push!(numorderslist, length(sp_orders))	
		...
		x_sp, y_sp, z_sp, w_sp, newobj_sp, solvetime_sp = solvesubproblem_lsns(subprobArcSet, subprobNodeSet, sp_orders, sp_drivers, sp_times, sp_compl_orders, sp_ordersinprogress, sp_Origin, sp_Destination, sp_containsextended, sp_trucksupply, sp_driversupply, x_currsol, y_currsol, z_currsol, w_currsol, ordtime_currsol, outsideorderdemand, sp_drivers_home)
		counter += 1
		println("------", counter, "------")
	end
end

writeresults_subproblemdata(subproblemdatafilename, improvementlist, improvementpctlist, solvetimelist, timewindowlist, numlocslist, numorderslist)
