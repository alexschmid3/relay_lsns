		...
		if originstarttime == 0
			...
			if a == 0
				...
				if endloc == nodesLookup[Destination[i][1]][1]
					push!(completedorders, i)
					global totalpastcost += lambda * ((availtime + totaldelta - orderOriginalStartTime[i]) - shortesttriptimes[i])/shortesttriptimes[i]
					global total_delay += ((availtime + totaldelta - orderOriginalStartTime[i]) - shortesttriptimes[i])/shortesttriptimes[i]
					global max_delay = max(max_delay, ((availtime + totaldelta - orderOriginalStartTime[i]) - shortesttriptimes[i])/shortesttriptimes[i] )
					remove!(ordersinprogress, i)
					orderdelayoutcomes[i] += availtime + totaldelta - orderOriginalStartTime[i]
				else
					#Update order origin to new location
					Origin[i] = [nodes[endloc, availtime - timedelta]]
