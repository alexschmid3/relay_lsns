...
include("code/lsns_randominstances.jl")
...
userandominstance_flag = 1
iterationordercap = ordercapslist[ex]					 
maxlocs = loclist[ex]
maxdrivers = driverlist[ex]								 
numtrucks = trucklist[ex]	
if userandominstance_flag == 1
	weekstart = DateTime(2019, 5, 26, 8)
	randomseedval = rand(1:999999)
	Random.seed!(randomseedval)
elseif userandominstance_flag == 0
	weekstart = DateTime(2019, 6, 2, 8) #weekstartlist[ex]
...
overlappct = 0.6
adjacentpct = 0.1
numlsnsiterations = rand(1:50)
println("numlsnsiterations = ", numlsnsiterations)

#Create id for this run
runid = string("initialsoln_ex", ex, "_id", instanceid, "_rundate", today())
...
writeorderoutcomesfiles = 0							# 1 = create the order outcomes file to see details of specific orders at specific iterations, 0 = do not
writecorridorfile = 0								# 1 = create the corridor file to see metrics on which arcs were used most often, 0 = do not
...
#Initial orders
if userandominstance_flag == 1
	orderwindowstart, orderwindowend = 0, becomesavailablehours
	origdestchooser = findorigindestinationdistribution("data/lh_data_isb_connect_clean.csv")
	numrandomorderschooser = findorderpertimeperioddistribution("data/lh_data_isb_connect_clean.csv")
	numorders, originloc, destloc, available, duedate, orderOriginalStartLoc, ordersinprogress = getrandomorders("data/lh_data_isb_connect_clean.csv", orderwindowstart, orderwindowend, origdestchooser, numrandomorderschooser, iterationordercap)
elseif userandominstance_flag == 0
...
for currtime in 0:timedelta:timedelta*(numiterations_online-1)
	...
	if userandominstance_flag == 1
		if currtime + timedelta + becomesavailablehours <= horizon
			getnextorders_randominstance(timedelta, currentdatetime, orders, "data/lh_data_isb_connect_clean.csv")
		else 
			newordersbyiter[currtime+timedelta] = 0
		end
	elseif userandominstance_flag == 0
...
objective_prelsns = lambda * sum((ordtime_currsol[i] - shortesttriptimes[i])/shortesttriptimes[i] for i in orders) + sum(sum(c[a]*x_currsol[i,a] for a in orderArcSet[i]) for i in orders) + sum(c[a]*y_currsol[a] for a in A_hasdriver) + sum(u[a]*w_currsol[a] for a in A_space)
println("Original objective = ", objective_prelsns)

for iter in 1:numlsnsiterations
	subprobindex = rand(1:length(subproblemlist))
	...
	if sp_orders == []
		...
		#Update the algorithm control parameters
		totalreoptimizations[subprobindex] += 1
		overlappingreoptimizations[subprobindex] = 0
		adjacentreoptimizations[subprobindex] = 0
		for osp in overlappingsubprobs[subprobindex] 
			overlappingreoptimizations[osp] = 1
		end
		for asp in adjacentsubprobs[subprobindex]
			adjacentreoptimizations[asp] = 1
		end
...
objective_postlsns = lambda * sum((ordtime_currsol[i] - shortesttriptimes[i])/shortesttriptimes[i] for i in orders) + sum(sum(c[a]*x_currsol[i,a] for a in orderArcSet[i]) for i in orders) + sum(c[a]*y_currsol[a] for a in A_hasdriver) + sum(u[a]*w_currsol[a] for a in A_space)
objectiveimprovement_lsns = round((objective_prelsns-objective_postlsns)/objective_prelsns, digits=4)
println("New objective = ", objective_postlsns, " (", objectiveimprovement_lsns*100, "% reduction)")
